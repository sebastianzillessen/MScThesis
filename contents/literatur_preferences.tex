...
$A \in \S$ is \textbf{acceptable} \gls{w.r.t.}	 $S'$ iff: $\forall B: B \rightarrow^{S'} A$, there is a $C \in S': C \rightarrow^{S'} B$ and there is a reinstatement set for $C \rightarrow^{S'} B$.
\end{definition}


\begin{figure}[htb]
	\centering
	\subfigure[0.3\textwidth][$A_1$ is not acceptable \gls{w.r.t.} $S' = \{A_1, A_2\}$]{
	...
	\hfill
	\subfigure[0.3\textwidth][$C$ is acceptable \gls{w.r.t.} $S' = \{A_1, A_2, A_3\}$]{
	...
	\hfill
	\subfigure[0.3\textwidth][$C$ is not acceptable \gls{w.r.t.} $S' = \{A_1, A_2, A_3\}$]{
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=1.2cm,
		                    thin]
...
\begin{exa}
	In Figure \autoref{fig:eaf_not_acceptable} $S'=\{A_1, A_2\}$ is not admissible since $A_1$ is not acceptable \gls{w.r.t.} $S'$. In Figure \autoref{fig:eaf_acceptable} $C$ is acceptable \gls{w.r.t.} $S' = \{A_1, A_2, A_3\}$ as there is a reinstatement set $\{A_1 \rightarrow^{S'} B_1, A_2 \rightarrow^{S'} B_2, A_3 \rightarrow^{S'} B_3\}$ for $A_1 \rightarrow^{S'} B_1$. In Figure \autoref{fig:eaf_not_acceptable_big} there is an additional argument $B_4$ such that $B_4 \rightarrow (A_3 \rightarrow B_3)$ and no argument in $S'$ that defeats\low{S'} $B_4$, then no reinstatement set for $A_1 \rightarrow^{S'} B_1$ would exist, hence $C$ is not acceptable \gls{w.r.t.} $S'$.
\end{exa}
...
\begin{definition}
	...
	\begin{itemize}
		\item $S'$ is an \textbf{admissible} extension iff every argument in $S'$ is acceptable \gls{w.r.t.} $S'$.
		\item $S'$ is a \textbf{preferred} extension iff $S'$ is (\gls{w.r.t.} set inclusion) a maximal admissible extension.
		\item $S'$ is a \textbf{complete} extension iff each argument which is acceptable \gls{w.r.t.} $S'$ is in $S'$.
...
\begin{lemma}
	Let $\Delta = \langle \S, \R, \D \rangle$ be an \gls{EAF}, $S'$ an admissible extension of $\Delta$ and let $A, A'$ be arguments which are acceptable \gls{w.r.t.} $S'$. Then:
	\begin{itemize}
		\item $S'' = S' \cup \{A\}$ is admissible.
		\item $A'$ is acceptable \gls{w.r.t.} $S''$.
	\end{itemize}	
\end{lemma}
...
\begin{lemma}
	...
	\begin{itemize}
		\item The set of all admissible extensions of $\Delta$ form a complete partial order \gls{w.r.t.} set inclusion.
